...
class TestSuite(unittest.TestCase):
    ...
    def test_interaction_is_logged_successful_infection(self):    
        eboola = Virus("Eboola", .7, .9)
        random_already_sick_person = Person(6, False, eboola)
        random_unvax_person = Person(5, False)
        self.test_logger.log_interaction(random_already_sick_person, random_unvax_person, did_infect=True)
        last_line = self.temp_file.readlines()[-1]
        self.assertEqual(last_line, f"{random_already_sick_person._id} infects {random_unvax_person._id}")
    ...
    def test_interaction_is_logged_unsuccessful_infection_because_of_vaccination(self):
        ...
        eboola = Virus("Eboola", .7, .9)
        random_already_sick_person = Person(6, False, eboola)
        self.test_logger.log_interaction(random_already_sick_person, random_vax_person, did_infect=False)
        last_line = self.temp_file.readlines()[-1]
        self.assertEqual(last_line, f"{random_already_sick_person._id} infects {random_vax_person._id}")

    def test_interaction_is_logged_unsuccessful_infection_because_already_sick(self):
        eboola = Virus("Eboola", .7, .9)
        person1 = Person(4, False, eboola)  
        random_already_sick_person = Person(6, False, eboola)
        self.test_logger.log_interaction(person1, random_already_sick_person, did_infect=False)
        last_line = self.temp_file.readlines()[-1]
        self.assertEqual(last_line, f"{person1._id} infects {random_already_sick_person._id}")
